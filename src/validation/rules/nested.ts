...
export function each<T, M extends object>(rulesInit: Rule<T, M> | Rules<T, M>): Rule<IterableType<T>, M> {
	const rules = Array.isArray(rulesInit) ? rulesInit : [rulesInit];
	return async (iterable, target) => {
...
async function checkSchema<T, M extends object>(schema: ValidatorSchema<T, M>, value: T, target: M): Promise<RuleCheckResult> {
	const nested: Partial<Record<string, RuleCheckResult>> = {};
...
export type ValidatorRef<M extends object> = Validator<M> | (() => Validator<M>) | BaseValidatorRawSchema<M>;
function derefValidatorRef<M extends object>(validatorRef: ValidatorRef<M>): ValidatorSchema<M, M> {
...
export function object<T, M extends object>(rawSchema: ValidatorRawSchema<T, M>): Rule<T, M> {
...
export function model<T extends object>(validatorRef: ValidatorRef<T>, keys?: Array<keyof T>): Rule<T> {
